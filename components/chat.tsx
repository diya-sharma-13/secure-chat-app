...
export default function Chat({ username, onLogout }: ChatProps) {
        ...
        try {
          ...
          if (msg.encrypted && msg.sender !== username) {
            try {
              ...
              console.log(`Decrypted message from ${msg.sender}: ${content}`)
            } catch (error) {
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try {
      ...
      let messageContent: string | { [username: string]: string } = newMessage
      ...
      if (otherUsers.length > 0) {
        try {
          ...
          if (newMessage.length > 100) {
            throw new Error("Message too long for encryption (max 100 characters)")
          }
          ...
          <div className="bg-white border-top p-3">
            <form onSubmit={handleSendMessage}>
              <div className="input-group">
                <input
                  ...
                  placeholder={cryptoReady ? "Type your message (max 100 chars)..." : "Setting up encryption..."}
                  value={newMessage}
            ...
            <div className="d-flex align-items-center justify-content-between mt-2">
              <small className="text-muted d-flex align-items-center">
                <LockIcon className="me-1" size={12} />
                Messages are encrypted end-to-end • {users.length} user(s) online
              </small>
              <small className="text-muted">{newMessage.length}/100</small>
            </div>
          </div>
        </div>
      </div>
